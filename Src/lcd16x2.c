...
void lcd16x2_showMenu(uint8_t position){
	switch(position){
	case 0:
		lcd16x2_clear();
		lcd16x2_setCursor(0, 4);
		if(_ActiveLight)
			lcd16x2_printf("Light <on>");
		else
			lcd16x2_printf("Light <off>");
	...
	case 3:
		...
		lcd16x2_printf("Weather");
		lcd16x2_setCursor(1, 4);
		lcd16x2_printf("TechnoPolis");
		break;
	}
}


void lcd16x2_shiftMarker(uint8_t position){
	if(position==0){
		lcd16x2_setCursor(0, 0);
		lcd16x2_printf("    ");
		lcd16x2_setCursor(1, 0);
