...
static uint8_t         i;
static uint8_t         txBufInd;
//static uint8_t         comand;
static uint8_t         txBuffer[8] = "";
static uint8_t         retstr[10];
static uint8_t         str[MFRC522_MAX_LEN]; //aktualnie wczytany id w postaci binarnej (4 bajty, ale miejsca na 16)
...
uint8_t SPI1SendByte(uint8_t data) {
        ...
        writeCommand[0] = data;
        HAL_SPI_TransmitReceive(&hspi2, (uint8_t*)&writeCommand, (uint8_t*)&readValue, 1, 100);
        return readValue[0];
}
...
bool MFRC522_IsDetected(void){
	if (!MFRC522_Request(PICC_REQIDL, str))				//PICC_REQIDL find the antenna area does not enter hibernation
...
static void char_to_hex(uint8_t data) {
...
char* MFRC522_ReturnHexKey(void){
	txBufInd = 0;
